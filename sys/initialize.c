...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	int kl, kj;
	for (kl = 0; kl < NUM_GLB_PG_TBLS; kl++)
	 {
	frame_t *pg_tbl = obtain_frame();
	pg_tbl->status = FRM_PGT;
	pg_tbl->fr_type = FRM_PGT;
	pg_tbl->fr_pid = NULLPROC;

		*(glb_pg_tbl_frm_mapping + kl) = pg_tbl->frm_num;
		for(kj = 0; kj< PAGE_TABLES ; kj++)
			add_pagetable(pg_tbl, kj, kl*PAGE_TABLES + kj);
	}


	frame_t *pg_dir = obtain_frame();
	create_directory(pg_dir, NULLPROC);
